...
class _ChangePasswordPageState extends State<ChangePasswordPage> {
  ...
  Future<void> _onSubmit() async {
    ...
    if (newPw.length < 6) {
      _showSnackBar('Kata sandi baru minimal 6 karakter.', AppColors.destructive);
      return;
    }
    if (newPw != confirm) {
      _showSnackBar('Kata sandi baru dan konfirmasi tidak cocok.', AppColors.destructive);
      return;
    }
    ...
    try {
      ...
      if (!mounted) return;
      _showSnackBar(e.toString().replaceFirst('Exception: ', ''), AppColors.destructive);
    } finally {
                ...
                child: Column(
                  ...
                  children: [
                    const Text(
                      ...
                      style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: AppColors.mutedForeground, height: 1.4),
                    ),
  ...
  Widget _buildInput({
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(label, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: AppColors.foreground)),
        ...
        Container(
          ...
          child: TextField(
            ...
            decoration: InputDecoration(
              hintText: hint,
              hintStyle: TextStyle(fontSize: 14, color: AppColors.mutedForeground.withOpacity(0.5)),
              contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 14),
              border: InputBorder.none,
              suffixIcon: IconButton(
                icon: Icon(obscure ? Icons.visibility_off_rounded : Icons.visibility_rounded, size: 20, color: AppColors.mutedForeground),
                onPressed: onToggle,
              ),
            ),
          ),
